...
void generateKeys(unsigned prime1, unsigned prime2, KeyPair *keyPairs_p) {
    ...
    unsigned testPublicKey = 0;
    //find key pairs till Array is full
    for (unsigned index = 0; index < MAX_EXP_KEYS; index++) {

        //calculate till a puplic key doesnt share a factor with coPrimeProduct gcd == 1
        do {
            ...
            while (check != 0) {
                unsigned interimResult = testPublicKey % check;
                testPublicKey = check;
                check = interimResult;
            }
        }
        while(testPublicKey != 1);

        //store the public key
        keyPairs_p->publicKey = publicKey;

        //create an array for the execution of the extended euclidean algorithm
        ...
        //store private key
        keyPairs_p->privateKey = calcPrivateKey[1][1];
        keyPairs_p++;
    }
}
